...
function sendMessage(guest, index) {
  ...
  const message =
    `Hello ${guest.Name}, Thanks for choosing www.egypttravelist.com as your transfer provider.%0A%0A` +
    `Your transfer from ${guest.From} to ${guest.To} on ${formatDate(
      guest.Date
    )} at ${formatTime(guest.Time)} for ${guest.pax} ${
      guest.pax === 1 ? "person" : "people"
    } is confirmed.%0A%0A` +
    ...
    `We also do all tours and trips inside Egypt.%0AHave a safe flight!%0A` +
    `EgyptTravelist: https://www.egypttravelist.com%0AThank you!`;
